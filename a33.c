...
int main ()
{
	...
	printf("\n input thr currency value (floating point with two decimal places): ");
	scanf("%lf",&amt);
	div(amt);
	return 0;
}
	int div(int amt)
{
	int int_amt ,frac_amt,amt,d;
	frac_amt=round((amt*100));
	printf("\bn currency notes:");
	printf("\n 100 number of notes: "%d,int_amt/100);
		...
		if(int_amt > 50)
	{
		...
		if(int_amt / 20>0)
		printf("\n 20 number of notes: "%d,int_amt/20);
